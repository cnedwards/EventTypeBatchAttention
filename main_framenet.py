...
from dataloader import load_zeroshot_data_BT_allmentions_FN
from networks import SentenceModel
from losses import type_representation_cos_ZS_loss
...
# Parameters
BATCH_SIZE = 10
SAVE_PATH = 'model_output/framenet/'
...
dataset = load_zeroshot_data_BT_allmentions_FN(source="../../../ACE05EN/source", bt_path = "backtranslations/", pretrained_model=MODEL_NAME, SBERT_embedding_file="SBERT_embeddings_allmentions.pkl")
...
for epoch in range(epochs):
    ...
    for i, d in enumerate(train_generator):
        ...
        frame_repres, subtype_oh, roles_oh, seen, EID, subtypes, all_roles_oh, SBERT_orig = labels
        frame_repres = frame_repres.to(device)
        ...
        #note that "trigger" is a prediction and not actually a trigger
        trep_loss = type_representation_cos_ZS_loss(trigger, frame_repres, seen).to(device)

        con_losses = tcon_loss + bt_tcon_loss + left_cross_tcon_loss + right_cross_tcon_loss
        loss = con_losses + trep_loss 

        running_loss += loss.item()
        running_losses[0] += tcon_loss.item()
        running_losses[1] += bt_tcon_loss.item()
        running_losses[2] += trep_loss.item()
...
torch.save(model.state_dict(), SAVE_PATH + "final_weights." + str(epochs)+".pt")

#wandb.finish()

plt.plot(train_losses)
plt.legend(['Train Type Contrastive', 'Train Type Regression', 'Auxiliary Loss', 'Test Type Contrastive', 'Test Type Regression'])
